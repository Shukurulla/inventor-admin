// src/components/CreateSpecificationForm.jsx - Admin panel uchun
...
const CreateSpecificationForm = ({
  ...
  useEffect(() => {
    if (isEdit && initialData) {
      // Handle disk specifications for computer types
      if (initialData.disk_specifications && initialData.disk_specifications.length > 0) {
        const existingStorages = initialData.disk_specifications.map(
          (disk, index) => ({
            id: disk.id || Date.now() + index,
          })
        );
        setStorageList(existingStorages);
        ...
        if (initialData.gpu_specifications && initialData.gpu_specifications.length > 0) {
          formValues.gpu_model = initialData.gpu_specifications[0].model;
        }

        // Clean up the form values
        delete formValues.disk_specifications;
        delete formValues.gpu_specifications;

        form.setFieldsValue(formValues);
      } else {
        // For non-computer types or when no disk specifications
    ...
    <Form form={form} layout="vertical" onFinish={handleSubmit}>
      ...
      <Row gutter={16}>
        <Col span={12}>
          <button
            type="button"
            className="w-100 p-2 rounded-[10px] font-semibold text-white block bg-gray-500 hover:bg-gray-600"
            style={{ width: "100%" }}
            onClick={onCancel}
          >
            Отмена
        ...
        <Col span={12}>
          <button
            type="submit"
            className="w-100 p-2 rounded-[10px] font-semibold text-white block bg-[#4E38F2] hover:bg-[#4A63D7]"
            style={{ width: "100%" }}
          >
